export async function initRegionsEngine(containerId) {
    ...
    // 1. Inject the pure layout for the Map and Data panes (EXACTLY AS YOU WROTE IT)
    container.innerHTML = `
    ...
    // Ensure Leaflet is loaded before running map code
    if (!window.L) {
    ...
    // Clean up container if navigating back and forth
    const mapContainer = window.L.DomUtil.get('map');
    if(mapContainer != null){ mapContainer._leaflet_id = null; }

    // 2. Initialize the Native Leaflet Map Engine (YOUR EXACT LOGIC)
    ...
    // 3. Fetch the Genesis Data via RPC and render YOUR boundaries
    window.nanbiDB.rpc('get_countries_geojson').then(res => {
