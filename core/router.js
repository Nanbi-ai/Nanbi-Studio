...
const UI_SHELL = `
  <style>
    /* Global fluid resets supporting 3:2 displays */
    ...
    /* Header Styles - Arrows Removed, Crumb moved left, Account moved right */
    ...
    /* Supabase-Style Hover Sidebar with separated icon boxes */
    ...
    /* Explicitly separating the Icon from the Label */
  ...
  <header class="app">
    ...
    <div class="header-actions">
      ...
      <button class="avatar-btn" title="Founder Account">N</button>
    </div>
  </header>
  ...
  <div class="app-body">
    <nav class="sidebar">
      ...
      <!-- System Settings pinned cleanly to the bottom left -->
      <a href="#/settings" class="bottom-pin" id="nav-settings">
        <span class="icon-box"><i class="fas fa-sliders-h"></i></span>
...
// 3. Core Routing Logic (Cleaned of hardcoded borders)
function router() {
    ...
    if (hash === "#/") {
        setCrumb("Home");
        renderView('<div class="surface-card p-5 flex-1 shadow-sm"><h2 class="text-xl font-bold text-main">Home</h2><p class="text-sm mt-2 text-sub">V5.0 Modular Architecture active. Layout optimized for 3:2 displays.</p></div>');
    } 
    else if (hash === "#/config") {
        ...
        setCrumb("Regions");
        renderView('<div class="surface-card p-5 flex-1 shadow-sm"><h2 class="text-xl font-bold text-main"><i class="fas fa-lock mr-2" style="color:var(--coral);"></i>Module Locked</h2><p class="text-sm mt-2 text-sub">Awaiting Configuration Engine initialization for dynamic Country > State > District taxonomy.</p></div>');
    }
    else if (hash === "#/settings") {
        setCrumb("System Settings");
        renderView('<div class="surface-card p-5 flex-1 shadow-sm"><h2 class="text-xl font-bold text-main">System Settings</h2><p class="text-sm mt-2 text-sub">Platform-level configurations and preferences will be managed here.</p></div>');
    }
}
