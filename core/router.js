// =======================================================================
// NANBI V5.0 MASTER ROUTER (100% DYNAMIC & ZERO HARDCODING)
// =======================================================================
...
function generateUIShell(appConfig) {
    return `
      <style>
        html, body { background:var(--bg, #F8FAFC); color:var(--text, #0F172A); font-size:var(--font-base, 16px); font-family:var(--font-main, 'Nunito', system-ui, sans-serif); font-weight:var(--weight-main, 400); height: 100vh; margin: 0; overflow: hidden; display: flex; transition: background 0.3s, color 0.3s; }
        ...
        .brand-text { font-family: var(--font-brand, 'Nunito', sans-serif); font-size: var(--brand-size, 1.25rem); font-weight: var(--weight-brand, 800); letter-spacing: -0.02em; margin-left: 12px; color: var(--brand-teal-dark, #2C4653); opacity: 0; transition: opacity 0.2s; }
        ...
        nav.sidebar a { display:flex; align-items:center; height: 44px; margin: 8px 12px; border-radius:var(--card-radius, 8px); color:var(--brand-teal-dark, #2C4653); text-decoration:none; font-weight: var(--weight-bold, 700); overflow: hidden; transition: background 0.15s; }
        ...
        .header-brand { font-family: var(--font-brand, 'Nunito', sans-serif); font-weight: var(--weight-brand, 800); font-size: var(--header-title-size, 1.15rem); color: var(--brand-teal-dark, #2C4653); letter-spacing: -0.02em; }
        .crumb-separator { font-size: 0.8rem; color: var(--muted, #64748B); margin: 0 16px; opacity: 0.6; }
        header.app-header .section-crumb { font-weight: var(--weight-bold, 700); font-size: 1.05rem; color: var(--text, #0F172A); }
        header.app-header .spacer { flex: 1; }
        
        .avatar-wrapper { position: relative; display: flex; align-items: center; }
        .avatar-btn { background: var(--brand-orange-dark, #D35400); color: #FFF; border: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; font-family: var(--font-brand, 'Nunito', sans-serif); font-weight: var(--weight-brand, 800); font-size: 0.85rem; display: flex; align-items: center; justify-content: center; margin-left: 8px; transition: transform 0.2s; }
        ...
        .dropdown-header .d-name { font-family: var(--font-brand, 'Nunito', sans-serif); font-weight: var(--weight-brand, 800); font-size: 0.95rem; color: var(--text, #0F172A); }
        .dropdown-header .d-email { font-size: 0.8rem; color: var(--muted, #64748B); }
        .dropdown-item { padding: 12px 16px; font-size: 0.9rem; font-weight: var(--weight-bold, 600); color: var(--text, #0F172A); text-decoration: none; display: flex; align-items: center; gap: 12px; transition: all 0.2s; cursor: pointer; }
...
function router() {
    try {
        ...
        if (hash === "#/") {
            ...
            renderView('<h2 style="font-size: var(--page-title-size, 1.875rem); font-weight: var(--weight-bold, 700); font-family: var(--font-brand); color: var(--page-title-color, var(--text));">' + (appLedger.page_home_title ?? '') + '</h2><p style="font-size: var(--page-subtitle-size, 1.125rem); margin-top: var(--spacing-sm, 8px); color: var(--page-subtitle-color, var(--muted));">' + (appLedger.page_home_subtitle ?? '') + '</p>');
        } 
        else if (hash === "#/config") {
            setCrumb(appLedger.nav_config_label ?? "");
            renderView('<div id="config-hub-container" class="w-full h-full"><p style="font-weight: var(--weight-bold, 700); color: var(--muted);">' + (appLedger.page_loading_text ?? 'Loading...') + '</p></div>');
            import('../modules/config_hub.js').then(m => m.initConfigHub('config-hub-container')).catch(e => {
                document.getElementById('config-hub-container').innerHTML = '<div style="color:red; font-weight:bold;">' + (appLedger.page_error_text ?? 'Error') + '</div>';
            });
        }
        else if (hash === "#/config/presentation") {
            setCrumb("Presentation Engine");
            renderView('<div id="config-pres-container" class="w-full h-full"><p style="font-weight: var(--weight-bold, 700); color: var(--muted);">' + (appLedger.page_loading_text ?? 'Loading...') + '</p></div>');
            import('../modules/config/presentation.js').then(m => m.initPresentationEngine('config-pres-container')).catch(e => {
                document.getElementById('config-pres-container').innerHTML = '<div style="color:red; font-weight:bold;">' + (appLedger.page_error_text ?? 'Error') + '</div>';
            });
        }
        else if (hash === "#/regions") {
            setCrumb(appLedger.nav_regions_label ?? "");
            renderView('<div style="background: var(--active-bg); border-left: 4px solid var(--brand-orange-dark); padding: var(--main-padding); border-radius: var(--card-radius);"><h2 style="font-size: var(--page-title-size, 1.875rem); font-family: var(--font-brand); font-weight: var(--weight-bold, 700); color: var(--brand-orange-dark);"><i class="fas fa-lock" style="margin-right: 12px;"></i>' + (appLedger.page_regions_title ?? '') + '</h2><p style="font-size: var(--page-subtitle-size, 1.125rem); margin-top: var(--spacing-sm, 8px); color: var(--page-subtitle-color, var(--text)); font-weight: var(--weight-main, 600);">' + (appLedger.page_regions_subtitle ?? '') + '</p></div>');
        }
        else if (hash === "#/settings") {
            setCrumb(appLedger.nav_settings_label ?? "");
            renderView('<h2 style="font-size: var(--page-title-size, 1.875rem); font-weight: var(--weight-bold, 700); font-family: var(--font-brand); color: var(--page-title-color, var(--text));">' + (appLedger.page_settings_title ?? '') + '</h2><p style="font-size: var(--page-subtitle-size, 1.125rem); margin-top: var(--spacing-sm, 8px); color: var(--page-subtitle-color, var(--muted));">' + (appLedger.page_settings_subtitle ?? '') + '</p>');
        }
        else if (hash === "#/account") {
            setCrumb("Account Profile");
            renderView('<h2 style="font-size: var(--page-title-size, 1.875rem); font-weight: var(--weight-bold, 700); font-family: var(--font-brand); color: var(--page-title-color, var(--text));">' + (appLedger.page_account_title ?? '') + '</h2><p style="font-size: var(--page-subtitle-size, 1.125rem); margin-top: var(--spacing-sm, 8px); color: var(--page-subtitle-color, var(--muted));">' + (appLedger.page_account_subtitle ?? '') + '</p>');
        }
    } catch (e) {
